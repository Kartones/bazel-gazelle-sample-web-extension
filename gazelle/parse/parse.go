...
func ParseJS(data []byte) ([]string, error) {
	...
	for _, block := range codeBlocks {
		blockImports, _, err := parseCodeBlock(data[block[0]:block[1]])
		if err != nil {
			return nil, err
		}
...
func GetDependencyName(bareSpecifier string) (string, error) {
	if extensionUtils.HasPrefixes(nonNPMPrefixes, bareSpecifier) {
		return bareSpecifier, fmt.Errorf("'%s' is not a bare specifier", bareSpecifier)
	}

	parts := strings.Split(bareSpecifier, "/")
	// namespaced bare specifier, e.g. "@mock/encore-web/..."
	if len(parts) > 1 && strings.HasPrefix(bareSpecifier, "@") {
		return strings.Join([]string{parts[0], parts[1]}, "/"), nil
	}

	// bare specifier, e.g. "foo/..."
	if len(parts) > 0 {
		return parts[0], nil
	}

	// simple bare specifier, e.g. "foo"
	return bareSpecifier, nil
}
